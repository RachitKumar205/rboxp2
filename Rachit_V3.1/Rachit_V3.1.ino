...
#define speedturn 65 // edit this to determine the speed during taking turns
...
#define Hspeedturn 60
#define led 13
#define button A5
#define obstacle 12
#define spd 80
#define casespd 80
#define specialspd 90
...
void follow_line()  //follow the line
{
  ...
  while (1)
  {
    ...
    line_position = qtrrc.readLine(sensorValues,QTR_EMITTERS_ON,1);
...
void turn(char dir)
{
  ...
  { case 'L':
      ...
      analogWrite(rightMotorPWM, Hspeedturn);
      analogWrite(leftMotorPWM, Hspeedturn);
      line_position = qtrrc.readLine(sensorValues,QTR_EMITTERS_ON,1);
...
void MazeSolve()
{
  ...
  { follow_line();
    ...
    { digitalLow( rightMotor2 );
      ...
      digitalLow( leftMotor1 );
      digitalHigh( leftMotor2 );
      analogWrite(rightMotorPWM, casespd);
      analogWrite(leftMotorPWM, casespd);
      for (int i = 0; i < 38; i++)
      {
        qtrrc.readLine(sensorValues);
        if (sensorValues[0] > threshold)
    ...
    { digitalLow( rightMotor2 );
      ...
      analogWrite(rightMotorPWM, casespd);
      analogWrite(leftMotorPWM, casespd);
      delay(105);                       //TIME  BEFORE TAKING RIGHT TURN DURING SEARCH RUN
    }

bailout:

    qtrrc.readLine(sensorValues,QTR_EMITTERS_ON,1);
    if (sensorValues[2] > threshold || sensorValues[3] > threshold || sensorValues[4] > threshold)
      found_straight = 1;
...
delta:
  ...
  while (1)
  {
    ...
    qtrrc.readLine(sensorValues);
    while(sensorValues[0] > threshold && sensorValues[6] > threshold){
        delay(500);
        digitalLow( rightMotor1 );
        digitalHigh( rightMotor2 );
        digitalLow( leftMotor2 );
    ...
    if( sensorValues[0] < threshold && sensorValues[6] < threshold ){
        ...
        digitalHigh( leftMotor2 );
        delay(100);
        while(sensorValues[2] < threshold || sensorValues[3] < threshold || sensorValues[4] < threshold)
        {
                  digitalLow( rightMotor1 );
                  digitalHigh( rightMotor2 );
                  digitalLow( leftMotor1 );
    ...
    for (int i = 0; i < path_length; i++)
    {
      ...
      { qtrrc.readLine(sensorValues,QTR_EMITTERS_ON,1);
        ...
        { digitalLow( rightMotor2 );
          ...
          digitalLow( leftMotor1 );
          digitalHigh( leftMotor2 );
          analogWrite(rightMotorPWM, specialspd);
          analogWrite(leftMotorPWM, specialspd);
          delay(140);
          qtrrc.readLine(sensorValues,QTR_EMITTERS_ON,1);
          if (sensorValues[2] < threshold && sensorValues[3] < threshold && sensorValues[4] < threshold)
          { digitalLow( rightMotor2 );
      ...
      { digitalLow( rightMotor2 );
        ...
        analogWrite(rightMotorPWM, casespd);
        analogWrite(leftMotorPWM, casespd);
        delay(100);                          //TIME  BEFORE TAKING RIGHT TURN DURING ACTUAL  RUN
      }
      else {
        digitalLow( rightMotor2 );
        digitalHigh( rightMotor1 );
        ...
        analogWrite(rightMotorPWM, casespd);
        analogWrite(leftMotorPWM, casespd);
        delay(105);                       //TIME  BEFORE TAKING LEFT TURN DURING ACTUAL RUN
      }
